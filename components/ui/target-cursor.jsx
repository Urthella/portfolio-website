...
const TargetCursor = ({
  ...
  // Where the cursor first appears (viewport coords). Read through a ref so it
  // only matters at mount and never re-runs the main effect.
  ...
  useEffect(() => {
    ...
    gsap.set(cursor, {
      xPercent: -50,
      yPercent: -50,
      x: (startPos?.x ?? window.innerWidth / 2) - initialOffset.x,
      y: (startPos?.y ?? window.innerHeight / 2) - initialOffset.y
    });
